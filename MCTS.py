...
class Node:
    ...
    def value(self, explore: float = MCTSMeta.EXPLORATION):
        if self.N == 0:
            ...
            return (
                self.Q / self.N + explore + math.sqrt(math.log(self.parent.N) / self.N)
            )
...
class MCTS:
    ...
    def back_propagate(self, node: Node, turn: int, outcome: int) -> None:
        # For current player, not next player
    ...
    def best_move(self):
        ...
        max_value = max(self.root.children.values(), key=lambda x: x.N).N
        max_nodes = [x for x in self.root.children.values() if x.N == max_value]
